    ...
    private void Update()
    {
        ...
        if (!currentStepTime.HasTimeLine())
        {
            foreach (var spawn in currentStepTime.toposToSpawn)
            {
                ...
                if (spawn.deltaSpawn >= (60 / spawn.countOfEnemyToSpawnOfMin))
                {
                    ...
                    var randomPosition = map.GetRandomPositionToTopo();
                    if (randomPosition == -1) continue;

                    var pointToTopo = map.GetPointToTopoByPosition(randomPosition);
                    if (pointToTopo == null)
                    {
                        Debug.LogWarning($"No se encontro PointToTopo para la posicion {randomPosition}");
                        continue;
                    }

                    var topo = factoryOfTopos.SpawnTopo(spawn.toposToSpawn, pointToTopo);
                    topo.StartTopo();
                    _topos.Add(topo);
                }
            }
        ...
        else
        {
            foreach (var timeLineStep in currentStepTime.GetTimeLine().GetSteps())
            {
                ...
                {
                    var pointToTopo = map.GetPointToTopoByPosition(timeLineStep.Position);
                    if (pointToTopo == null)
                    {
                        Debug.LogWarning("No hay posicion disponible para el paso de timeline en este frame");
                        continue;
                    }

                    var topo = factoryOfTopos.SpawnTopo(timeLineStep.GetTopo(), pointToTopo);
                    topo.StartTopo();
                    _topos.Add(topo);
